...
$(() => {
  ...
  const onClose = () =>
    setTimeout(() => ReactDOM.unmountComponentAtNode($modalPortal[0]))
  $('#newEvent').on('click', () => {
    ReactDOM.render(
      <NewEventForm candidate="Test Candidate" onRequestClose={onClose} />,
      $modalPortal[0]
    )
  })
})
...
Api.get.events().then(data => {
  ...
  request
    .get('//d1y0otadi3knf6.cloudfront.net/d/us_postal_codes.gz')
    .end((err, res) => {

      const zipcodes = csvParse(res.text)
      $('#loading-icon').hide()
      ...
      window.EVENT_DATA.forEach(function(d) {
        ...
        switch (d.event_type) {
          case 'Facebook Event':
            d.filters.push('facebook-event')
            break
          case 'Event':
            d.filters.push('event')
            break
          case 'Group Meeting':
            d.filters.push('group-meeting')
            break
          case 'Group':
      ...
      if (m && m[1] && m[2] && m[3]) {
        ...
        var params = $.deparam
        window.mapManager.filterByCoords(
          defaultCoord.center,
          50,
          params.sort,
          params.f
...
/* Prepare filters */
const renderedFilters = window.eventTypeFilters.map(function(d) {
  return $('<li />')
    .append(
        ...
        .append(
          $('<span />')
            ...
            .append(
              d.onItem
                ? d.onItem
                : $('<span>').addClass('circle-button default-on')
            )
        )
        .append(
          $('<span />')
            ...
            .append(
              d.offItem
                ? d.offItem
                : $('<span>').addClass('circle-button default-off')
            )
        )
...
renderedFilters.forEach(el => target.append(el))
/***
 *  define events
 */
//only numbers
...
$(window).on('hashchange', function(e) {
  ...
  setTimeout(function() {
    ...
    if (
      window.mapManager._options &&
      window.mapManager._options.defaultCoord &&
      params.zipcode.length != 5
    ) {
      window.mapManager.filterByType(params.f)
      window.mapManager.filterByCoords(
        window.mapManager._options.defaultCoord.center,
        params.distance,
        params.sort,
        params.f
      )
...
if ($('body').hasClass('initial-view')) {
  if (
    $(window).width() >= 600 &&
    (!pre.zipcode || (pre && pre.zipcode.length != 5))
  ) {
